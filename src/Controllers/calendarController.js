...
const uploadImages = async (req, res) => {
  ...
  const url = files["url"] ? `${PUBLIC_URL}/Storage/${files["url"][0].filename}` : null;
  const images = files["images"] ? files["images"].map((file) => `${PUBLIC_URL}/Storage/${file.filename}`) : [];
  ...
  const fileData = {
    ...
    url: url,
    images: images,
    filename: files["images"] ? files["images"].map((file) => file.filename).join(",") : "",
  };
  try {
